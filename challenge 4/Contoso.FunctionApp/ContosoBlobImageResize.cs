...
{
    public class ContosoBlobImageResize
    {
        private readonly ILogger<ContosoBlobImageResize> _logger;

        public ContosoBlobImageResize(ILogger<ContosoBlobImageResize> logger)
        {
            _logger = logger;
        }

        [Function(nameof(ContosoBlobImageResize))]
        public async Task Run(
            // Connection is set to an empty string because the AzureWebJobsStorage connection string is used by default
            [BlobTrigger("contosoimages/{name}", Connection = "")] Stream stream, 
            string name)
        {
